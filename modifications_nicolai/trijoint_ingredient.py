...
class im2recipe(nn.Module):
    ...
    def forward(self, x, y1, y2, z1, z2):  # we need to check how the input is going to be provided to the model
        zeros_st_rnn = torch.zeros([1, opts.srnnDim], dtype=torch.float32, device='cuda:0')
        zeros_st_rnn_type = zeros_st_rnn.type()
        ingredients_nn = self.ingRNN_(z1, z2)
        ingredients_nn_type = ingredients_nn.type()

        recipe_emb = self.table.forward([zeros_st_rnn, ingredients_nn], 1)  # joining on the last dim
        recipe_emb = self.recipe_embedding(recipe_emb)
        # print('recipe_emb: ' + str(recipe_emb))
        recipe_emb = norm(recipe_emb)
        # print('recipe_emb_normed: ' + str(recipe_emb))

        visual_emb = torch.zeros([1, opts.srnnDim], dtype=torch.float32, device='cuda:0')

        if opts.semantic_reg:
            visual_sem = visual_emb
